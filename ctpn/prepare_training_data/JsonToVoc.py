...
import cv2 as cv
import json
path = '/Users/wangrui/Downloads/aaa/new/images'
gt_path = '/Users/wangrui/Downloads/aaa/new/annotations'
out_path = 're_image'
...
for file in files:
    ...
    gt_file = os.path.join(gt_path, stem + '.json')
    ...
    for line in data['shapes']:
        ...
        pt_x = np.zeros((4, 1))
        pt_y = np.zeros((4, 1))
        pt_x[0, 0] = int(float(points[0][0]) / img_size[1] * re_size[1])
        pt_y[0, 0] = int(float(points[0][1]) / img_size[0] * re_size[0])
        pt_x[1, 0] = int(float(points[1][0]) / img_size[1] * re_size[1])
        pt_y[1, 0] = int(float(points[1][1]) / img_size[0] * re_size[0])
        pt_x[2, 0] = int(float(points[2][0]) / img_size[1] * re_size[1])
        pt_y[2, 0] = int(float(points[2][1]) / img_size[0] * re_size[0])
        pt_x[3, 0] = int(float(points[3][0]) / img_size[1] * re_size[1])
        pt_y[3, 0] = int(float(points[3][1]) / img_size[0] * re_size[0])
